...
const AllCourses: FC<Props> = (props) => {
  ...
  const modalHeadingRef = useSpeechOnHover<HTMLHeadingElement>("Are you sure you want to delete this course?");
  ...
  const columns = [
    ...
    { field: "rating", headerName: "Rattings", flex: 0.5 },
    ...
    { field: "created_at", headerName: "Created At", flex: 0.5 },
    {
      ...
      renderCell: (params: any) => {
        const editRef = useSpeechOnHover<HTMLAnchorElement>(`Edit course ${params.row.title}`);
        return (
          <>
            <Link
              ref={editRef}
    ...
    },
    {
      ...
      flex: 0.2,
      renderCell: (params: any) => {
        const deleteRef = useSpeechOnHover<HTMLButtonElement>(`Delete course ${params.row.title}`);
        return (
          <>
            <Button
              ref={deleteRef}
          ...
          {open && (
            ...
            >
              <Box className="absolute top-[50%] left-[50%] -translatex-1/2 -translate-y-1/2 w-[450px] bg-white dark:bg-slate-900 rounded-[8px] shadow p-4 outline-none">
                <h1 ref={modalHeadingRef} tabIndex={0} className={`${styles.title}`}>
