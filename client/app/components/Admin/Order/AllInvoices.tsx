...
// Email Invoice Link Component with speech on hover
const EmailInvoiceLink: FC<{ email: string }> = ({ email }) => {
  const emailRef = useSpeechOnHover<HTMLAnchorElement>(`Send email to ${email}`);
  return (
    <a ref={emailRef} tabIndex={0} href={`mailto:${email}`}>
      <AiOutlineMail
      ...
      : [
          {
            ...
            renderCell: (params: any) => {
              return (
                <EmailInvoiceLink email={params.row.userEmail} />
              );
            },
          },
        ]),
