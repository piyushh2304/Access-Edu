...
const DashboardWidgets: FC<Props> = ({ open }) => {
  ...
  const salesObtainedRef = useSpeechOnHover<HTMLDivElement>(`Sales obtained: ${orderComparePrecentenge?.currentMonth || 0}`);
  const salesPercentRef = useSpeechOnHover<HTMLHeadingElement>(`Sales change: ${orderComparePrecentenge?.percentChange > 0 ? '+' : ''}${orderComparePrecentenge?.percentChange?.toFixed(2) || 0} percent`);
  const newUsersRef = useSpeechOnHover<HTMLDivElement>(`New users: ${userComparePrecentenge?.currentMonth || 0}`);
  const usersPercentRef = useSpeechOnHover<HTMLHeadingElement>(`Users change: ${userComparePrecentenge?.percentChange > 0 ? '+' : ''}${userComparePrecentenge?.percentChange?.toFixed(2) || 0} percent`);
  const recentTransactionsRef = useSpeechOnHover<HTMLHeadingElement>("Recent transactions");
    ...
    <div className="mt-[50px] min-h-screen">
      <div className="grid grid-cols-[75%,25%]">
        ...
        <div className="pt-[80px] pr-8">
          <div ref={salesObtainedRef} tabIndex={0} className="w-full dark:bg-[#111c43] rounded-sm shadow">
            <div className="flex items-center p-5 justify-between">
              ...
              <div>
                <CircularProgressWithLabel
                  ...
                  open={open}
                />
                <h5 ref={salesPercentRef} tabIndex={0} className="text-center pt-4">
          ...
          <div ref={newUsersRef} tabIndex={0} className="w-full dark:bg-[#111c43] rounded-sm shadow my-8">
            <div className="flex items-center p-5 justify-between">
              <div className="">
              ...
              <div>
                <CircularProgressWithLabel
                  ...
                  open={open}
                />
                <h5 ref={usersPercentRef} tabIndex={0} className="text-center pt-4">
      ...
      <div className="grid grid-cols-[65%,35%] mt-[-20px]">
        ...
        <div className="p-5">
          <h5 ref={recentTransactionsRef} tabIndex={0} className="dark:text-[#fff] text-black text-[20px] font-[400] font-Poppins pb-3">
            Recent Transactions
          </h5>
